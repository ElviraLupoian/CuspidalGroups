// this code bounds the size of the torsion subgroup of J1(p) over the maximal real subfield of the pth cyclotomic field , and compares this to the order  cuspidal subgroup  


P := PrimesInInterval(11,79) ;
...
KK := CyclotomicField(p) ;
SKK := Subfields(KK) ;
s := (p-1)/2;
s := Integers() ! s ;
SK1 := [ a[1] : a in SKK | Degree(a[1]) eq s];
K1 := SK1[1];

J := JOne(p) ;
JK :=  BaseExtend(J, K1) ;

torsbound := TorsionMultiple(JK) ;
<p, #CS, torsbound>;
end for ;
